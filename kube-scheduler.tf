...
resource "shell_script" "kube-scheduler-bin" {
  lifecycle_commands {
    create = <<-EOF
        mkdir -p bin
        wget -q --https-only --timestamping "https://storage.googleapis.com/kubernetes-release/release/${var.kubernetes_version}/bin/linux/amd64/kube-scheduler" -O bin/kube-scheduler
        chmod +x bin/kube-scheduler
    EOF
    read   = <<-EOF
        echo "{\"md5\": \"$(md5sum bin/kube-scheduler|base64)\"}"
    EOF
